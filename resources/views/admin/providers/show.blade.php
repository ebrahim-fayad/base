...
@section('css')
    ...
    <style>
        ...
        /* تمييز ألوان التابات: البيانات الأساسية - الشكاوى - الطلبات - الخدمات - تحديثات المزود */
        #user-profile .provider-tabs .tab-link {
            border-bottom: 3px solid transparent;
            margin-bottom: -1px;
            padding-bottom: 0.5rem;
            transition: color 0.2s, border-color 0.2s;
        }
        #user-profile .provider-tabs .tab-link:hover {
            opacity: 0.85;
        }
        /* البيانات الأساسية - أزرق */
        #user-profile .provider-tabs .tab-main_data.active,
        #user-profile .provider-tabs .tab-main_data:hover { color: #7367f0 !important; }
        #user-profile .provider-tabs .tab-main_data.active { border-bottom-color: #7367f0; }
        /* الشكاوى - برتقالي */
        #user-profile .provider-tabs .tab-complaints.active,
        #user-profile .provider-tabs .tab-complaints:hover { color: #ff9f43 !important; }
        #user-profile .provider-tabs .tab-complaints.active { border-bottom-color: #ff9f43; }
        /* الطلبات - بنفسجي */
        #user-profile .provider-tabs .tab-orders.active,
        #user-profile .provider-tabs .tab-orders:hover { color: #9c27b0 !important; }
        #user-profile .provider-tabs .tab-orders.active { border-bottom-color: #9c27b0; }
        /* المحفظة - أخضر */
        #user-profile .provider-tabs .tab-wallet.active,
        #user-profile .provider-tabs .tab-wallet:hover { color: #28c76f !important; }
        #user-profile .provider-tabs .tab-wallet.active { border-bottom-color: #28c76f; }
        /* الخدمات - تركواز */
        #user-profile .provider-tabs .tab-services.active,
        #user-profile .provider-tabs .tab-services:hover { color: #00cfe8 !important; }
        #user-profile .provider-tabs .tab-services.active { border-bottom-color: #00cfe8; }
        /* تحديثات المزود - كهرماني */
        #user-profile .provider-tabs .tab-provider_updates.active,
        #user-profile .provider-tabs .tab-provider_updates:hover { color: #ff9f43 !important; }
        #user-profile .provider-tabs .tab-provider_updates.active { border-bottom-color: #ff9f43; }
    </style>
@endsection
    ...
    <div class="app-content content m-0">
        ...
        <div class="content-wrapper m-0 p-0">
            <div class="content-body">
                <div id="user-profile">
                    @include('admin.providers.parts.links')

                    <section id="profile-info">
                        <div class="row">

                            <div class="col-lg-8 col-12 refrshed-data ">
                                @include('admin.providers.parts.main_data')

                            </div>

                            <div class="col-lg-4 col-12">
                                @if ($row->is_approved == ApprovementStatusEnum::PENDING->value)
                                    @include('admin.providers.parts.approvement_actions')
                                @endif
                                @include('admin.providers.parts.charge_wallet')
                                @include('admin.providers.parts.notify')
                            </div>
...
@section('js')
    ...
    @include('admin.providers.parts.change_provider_approvement', [
        'route' => route('admin.providers.toggleApprovement'),
    ])
    ...
    <script>
        ...
        $(document).on('click', '.show-details-links li a.tab-link', function(e) {
            e.preventDefault()
            var url = $(this).data('href')
            if (!url) return
            $('.provider-tabs .tab-link').removeClass('active')
            ...
            $.ajax({
                ...
                success: (response) => {
                    ...
                    if (response.balance !== undefined) {
                        $('.available_balance').html(Math.round(response.balance) + ' ' + currency);
                    }
